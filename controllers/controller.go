...
func (c *control) NewUser(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode([]byte(result))
}
...
func (c *control) GetUser(w http.ResponseWriter, r *http.Request) {
	...
	result := fmt.Sprintf("Found user %s, %d, %s", account.Name, account.Age, account.Number)
	json.NewEncoder(w).Encode([]byte(result))
...
func (c *control) DeleteUser(w http.ResponseWriter, r *http.Request) {
	...
	result := fmt.Sprintf("Deleted user %s, %d, %s", account.Name, account.Age, account.Number)
	json.NewEncoder(w).Encode([]byte(result))
}
